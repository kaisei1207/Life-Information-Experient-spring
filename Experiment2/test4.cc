    ...
    const vector<int>& y)
{
    ...
    // ===== Precision（適合率）=====
    double prec;
    if (TP + FP == 0) prec = 0;   // 分母0対策
    else prec = (double)TP / (TP + FP);

    // ===== Recall（再現率）=====
    double rec;
    if (TP + FN == 0) rec = 0;
    else rec = (double)TP / (TP + FN);
    ...
    cout << "feature_id: " << tree[0].feature_id << endl;
    cout << "threshold: " << tree[0].threshold << endl;
...
int main(){
    ...
    // ===== 学習 =====
    // training_dataset と training_labels を使って
    // 「最も良い分割（特徴量・閾値）」を決定し
    // decision_tree に学習結果を保存する
    TrainDecisionTree(training_dataset, training_labels, decision_tree);
    
    // ===== 評価 =====
    // 学習済みの decision_tree を使って
    // test_dataset に対して予測を行い
    // Accuracy / Precision / Recall / F1 / 混同行列を計算する
